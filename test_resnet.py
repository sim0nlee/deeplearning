...
model = ResNet(model_parameters['resnet50'],
               ...
               shortcut_weight=0.8,
               activation_name="trelu").to(device)
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=0.001)
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.1)

# Set up TensorBoard
graph_writer = SummaryWriter('logs/imagenet/shortcut_trelu/resnet50/graph')
writer = SummaryWriter('logs/imagenet/shortcut_trelu/resnet50/train')
test_writer = SummaryWriter('logs/imagenet/shortcut_trelu/resnet50/test')
grad_writer = SummaryWriter('logs/imagenet/shortcut_trelu/resnet50/grad')
dummy_input = torch.randn(1, 3, 224, 224).to(device)
graph_writer.add_graph(model, dummy_input)

# Save checkpoints
checkpoint_dir = 'checkpoints/imagenet/shortcut_trelu/resnet50'
...
model_save_path = os.path.join(models_dir, 'resnet50_shortcut_trelu.pth')
